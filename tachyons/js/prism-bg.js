/*
  Prism background — ported from the React Bits <Prism /> component
  (https://reactbits.dev/backgrounds/prism) to plain JS/WebGL so it can
  run without React or a build step. Original component by David Haz,
  MIT + Commons Clause licensed. Uses the `ogl` WebGL micro-library.
*/
...
function initPrism(container, opts = {}) {
  const {
    height = 3.5,
    baseWidth = 5.5,
    animationType = 'rotate', // 'rotate' | '3drotate' | 'hover'
    glow = 1,
    offset = { x: 0, y: 0 },
    noise = 0.5,
    transparent = true,
  ...
  if (animationType === 'hover') {
    window.addEventListener('pointermove', e => { onMove(e); startRAF(); }, { passive: true });
...
// ---- Mount on the page's background container ----
...
if (container && window.WebGLRenderingContext) {
  ...
  try {
    initPrism(container, {
      ...
      timeScale: reduceMotion ? 0 : 0.8
    });
  } catch (err) {
    console.warn('Prism background failed to initialize:', err);
  }
}